		...
		try {
			if (isOnline()) {
				while (!stopThis) {
					synchronized (this) {
						try {
 							Log.d(CATEGORIA,module+" PING "+ ipAddress+" "+wifiInterface);
 							ping.nqm=0.0;
							ping.runParser(ipAddress, 10, wifiInterface);
